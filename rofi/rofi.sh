...
##### MAIN_MENU ####
    ...
    main_menu_cmds=(
        'feh --randomize --bg-fill ~/wm/wallpaper/*.png; show_main_menu' # 执行完不退出脚本继续执行show_main_menu
        'coproc ($HOME/wm/config/dwm/statusbar/statusbar.sh updateall > /dev/null 2>&1); show_main_menu'
...
##### TOGGLE_SERVER_MENU #####
    ...
    toggle_server_menu_cmds[1]='coproc ($HOME/wm/config/dwm/statusbar/statusbar.sh updateall > /dev/null)'
